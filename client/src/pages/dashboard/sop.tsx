...
import { Plus, Search, Edit, Trash2, FileText, Upload, X } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
...
export default function SOPPage() {
  ...
  const [sopForm, setSOPForm] = useState({
    title: "",
    department: "",
    segments: [{ title: "", content: "", fileUrl: null, segmentOrder: 0 }] as SOPSegment[]
  });

  // Check if user has access
  const hasAccess = user?.role === "operations_manager" || user?.specialization === "operations_manager";
  ...
  }, [hasAccess, selectedDepartment, searchTerm]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!sopForm.title || !sopForm.department || sopForm.segments.length === 0) {
      toast({
        title: "Error",
        description: "Please fill in all required fields",
        variant: "destructive",
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: `SOP ${editingSOP ? "updated" : "created"} successfully`,
        });
        ...
        throw new Error(`Failed to ${editingSOP ? "update" : "create"} SOP`);
      }
    } catch (error) {
      console.error("Error saving SOP:", error);
      toast({
        title: "Error",
        description: `Failed to ${editingSOP ? "update" : "create"} SOP`,
        variant: "destructive",
      });
    }
  };
  ...
  const resetForm = () => {
    setSOPForm({
      title: "",
      department: "",
      segments: [{ title: "", content: "", fileUrl: null, segmentOrder: 0 }]
    });
  };
  ...
  const addSegment = () => {
    setSOPForm(prev => ({
      ...
      segments: [...prev.segments, { 
        ...
        content: "", 
        fileUrl: null, 
        segmentOrder: prev.segments.length 
      }]
    }));
  };

  const removeSegment = (index: number) => {
    if (sopForm.segments.length > 1) {
      setSOPForm(prev => ({
        ...prev,
        segments: prev.segments.filter((_, i) => i !== index)
      }));
  ...
  const updateSegment = (index: number, field: keyof SOPSegment, value: string | number) => {
    setSOPForm(prev => ({
      ...prev,
      segments: prev.segments.map((segment, i) => 
        i === index ? { ...segment, [field]: value } : segment
      ...
      <div className="container mx-auto p-6">
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <p className="text-gray-500">Only Operations Managers can access Standard Operating Procedures.</p>
          </CardContent>
        </Card>
      </div>
    );
    ...
    <div className="container mx-auto p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-600">Manage and organize company SOPs</p>
        </div>
        <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
          <DialogTrigger asChild>
            <Button onClick={() => { resetForm(); setEditingSOP(null); }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-4 mt-2">
                  {sopForm.segments.map((segment, index) => (
                    <Card key={index}>
                      <CardHeader className="pb-4">
                        <div className="flex justify-between items-center">
                          <h4 className="font-medium">Segment {index + 1}</h4>
                          {sopForm.segments.length > 1 && (
                            <Button
                              type="button"
                              variant="ghost"
                              size="sm"
                      ...
                      <CardContent className="space-y-4">
                        <div>
                          <Label htmlFor={`segment-title-${index}`}>Segment Title</Label>
                          <Input
                        ...
                        <div>
                          <Label htmlFor={`segment-content-${index}`}>Content</Label>
                          <Textarea
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="outline" onClick={() => handleDelete(sop.id)}>
                ...
                <div className="space-y-4">
                  {sop.segments.map((segment, index) => (
                    <div key={segment.id || index}>
                      ...
                      <p className="text-sm text-gray-600 mb-2">{segment.content}</p>
                      {segment.fileUrl && (
                        <a
                          href={segment.fileUrl}
                          target="_blank"
                      ...
                      {index < sop.segments.length - 1 && <Separator className="mt-4" />}
                    </div>
                  ))}
                </div>
