...
const getStatusLabel = (task: any, workingHours?: number, workingMinutes?: number) => {
  if (!task || typeof task.timeSpent !== 'number') {
    return 'Invalid Data';
  }
...
export default function ProductivityPage() {
  ...
  const taskPieData = productivityData?.today?.taskBreakdown?.filter(task => 
    ...
    const workingHours = (task as any).workingHours || 0; // Default to 0 hours if not provided
    const workingMinutes = (task as any).workingMinutes || 0; // Default to 0 minutes if not provided
    ...
    return {
      ...
      color: getTaskStatusColor(task, workingHours, workingMinutes),
      statusLabel: getStatusLabel(task, workingHours, workingMinutes),
      workingHours: workingHours,
      workingMinutes: workingMinutes,
      timeUsedPercentage: (workingHours * 3600 + workingMinutes * 60) > 0 ? Math.round((task.timeSpent / ((workingHours * 3600) + (workingMinutes * 60))) * 100) : 0
    };
  }) || [];
        ...
        <div className="flex-1 overflow-auto p-6">
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Productivity Tracking</h1>
                      ...
                      <PieChart>
                        ...
                        <Tooltip 
                          formatter={(value: number, name: string, props: any) => {
                            ...
                            const allocatedTime = (props?.payload?.workingHours || 0) * 3600 + (props?.payload?.workingMinutes || 0) * 60;
                            return [
                              formatTime(value || 0), 
                              `Time Spent (${timeUsedPercentage}% of ${formatTime(allocatedTime)})`
                            ];
                          }}
                          labelFormatter={(label, payload) => {
                            if (payload && payload.length > 0 && payload[0].payload) {
                              ...
                              const allocatedTime = (workingHours * 3600) + (workingMinutes * 60);
                          ...
                          <TableCell>
                            <div className="flex flex-col gap-1">
                              ...
                              {(() => {
                                const workingHours = (task as any).workingHours || 0;
                                const workingMinutes = (task as any).workingMinutes || 0;
                                const allocatedTimeInSeconds = (workingHours * 3600) + (workingMinutes * 60);
                                const timeUsedPercentage = allocatedTimeInSeconds > 0 ? (task.timeSpent / allocatedTimeInSeconds) * 100 : 0;
                                const statusLabel = getStatusLabel(task, workingHours, workingMinutes);
                                const statusColor = 
