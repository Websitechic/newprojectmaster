...
export default function TechnicalManagementPage() {
  ...
  const filteredAndSortedRequests = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      ...
      if (typeof aValue === "string") { // Check for string dates
        const dateA = new Date(aValue).getTime();
        const dateB = new Date(bValue).getTime();
        return sortOrder === "asc" ? dateA - dateB : dateB - dateA;
      }

      // Added fallback for non-string/date types, although not strictly necessary for current keys
  ...
  return (
    <div className="flex h-screen w-full">
      <Sidebar currentPath="/dashboard/technical-management" />
