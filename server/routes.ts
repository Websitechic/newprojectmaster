...
import {
  ...
  staffQueries,
  notes,
  sops, // Import SOPs schema
  sopSegments, // Import SOP Segments schema
} from "@db/schema";
...
<body>
  ...
  app.post("/api/kpi-report/export", async (req, res) => {
    ...
    try {
      ...
      if (format === 'csv') {
        // Generate CSV
        const csvHeader = 'Date,Total Hours,Performance Status,Task Count,Tasks\n';
...
The following changes add the necessary SOP API routes, including file upload configuration and CRUD operations for SOPs and their segments.
<replit_final_file>
import { Express, Response, Request, NextFunction } from "express";
