...
// Helper function to emit SSE event to a specific user
function emitToUser(userId: number, message: any) {
  if (global.sseClients && global.sseClients.has(userId)) {
    const client = global.sseClients.get(userId);
    if (client && !client.writableEnded) {
      try {
        client.write(`data: ${JSON.stringify(message)}\n\n`);
        console.log(`📤 SSE event emitted to user ${userId}:`, message.type);
      } catch (error) {
        console.error(`❌ Error emitting SSE event to user ${userId}:`, error);
        global.sseClients.delete(userId);
      }
    } else {
      console.log(`⚠️ SSE client for user ${userId} is not writable or ended when emitting event.`);
    }
  } else {
    console.log(`⚠️ No active SSE client found for user ${userId} to emit event.`);
  }
}
...
==================================================`;
  ...
  app.post("/api/direct-messages", async (req, res) => {
    ...
    try {
      ...
      const messageWithSender = {
        ...newMessage,
        senderName: user.username || user.name, // Use username if available, otherwise name
      ...
      if (replyToMessageId && replyToSenderName) {
        ...
        const repliedToUser = await db
          .select()
          .from(users)
          .where(eq(users.name, replyToSenderName)) // Use name for lookup, as senderName is used above
          .limit(1);
      ...
      // Send SSE event for direct message
      const sseMessageToReceiver = {
        type: 'direct_message',
        data: messageWithSender
      };
      console.log('📤 SSE: Emitting direct message event to receiver:', {
        messageId: newMessage.id,
        to: parseInt(receiverId),
        from: senderId
      });
      emitToUser(parseInt(receiverId), sseMessageToReceiver);

      // Also send SSE event to sender for their own UI updates
      const sseMessageToSender = {
        type: 'direct_message',
        data: {
          ...messageWithSender,
          senderName: user.username || user.name // Ensure senderName is included for sender too
        }
      };
      console.log('📤 SSE: Emitting direct message event to sender:', {
        messageId: newMessage.id,
        to: senderId,
        from: senderId
      });
      emitToUser(senderId, sseMessageToSender);
  ...
  app.get("/api/notifications/stream", (req, res) => {
    ...
    const connectedMessage = JSON.stringify({ type: "connected", userId });
    console.log('📤 SSE: Sending connected message to user:', userId);
...
<changes>
Add debug logging for SSE stream endpoint
Replacing
<old_str>
// SSE endpoint for real-time notifications
...
Add SSE event emission for direct messages
Replacing
<old_str>
// Create the message
    ...
    broadcastDirectMessage(newMessage);
</old_str>
with
<new_str>
// Create the message
    ...
    // Emit SSE event for direct message
    ...
    console.log('📤 SSE: Emitting direct message event:', {
      messageId: newMessage.id,
      to: receiverId,
      from: req.user.id
    });

    emitToUser(receiverId, sseMessage);
    emitToUser(req.user.id, sseMessage); // Also send to sender for their own UI update
</new_str>
</changes>
<replit_final_file>
import { Express, Response, Request, NextFunction } from "express";
