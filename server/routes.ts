...
==================================================`;
  ...
  app.put("/api/project-plans/:id", async (req, res) => {
    ...
    try {
      ...
      if (planDeliverables && planDeliverables.length > 0) {
        const deliverableData = planDeliverables.map((deliverable: any, index: number) => ({
          projectPlanId: planId,
          name: deliverable.name || `Deliverable ${index + 1}`,
          description: deliverable.description || "",
          startDate: deliverable.startDate ? new Date(deliverable.startDate) : parsedStartDate,
          endDate: deliverable.endDate ? new Date(deliverable.endDate) : parsedEndDate,
          duration: deliverable.duration || 1,
          status: "pending",
          order: deliverable.order || index,
        }));
