...
<changes>
Fix team message sending by retrieving message with sender info after insert
Replacing
<old_str>
const [newMessage] = await db
        .insert(projectMessages)
...
// Delete project
  ...
  app.post("/api/projects/:id/plans", async (req, res) => {
    ...
    try {
      ...
      // Parse and validate dates
      const parsedStartDate = new Date(startDate);
      const parsedEndDate = new new Date(endDate);
  ...
  app.post("/api/tasks/:id/pause-timer", async (req, res) => {
    ...
    try {
      ...
      // Calculate session duration
      const sessionDuration = Math.floor((Date.now() - new Date(task.timerStartTime).getTime()) / 1000);
      const newTimeSpent = (task.timeSpent || 0) + sessionSeconds;
